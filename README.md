...
Audit-validated precision: **96.2%** (95% CI: 94.5%–97.9%) — measured 2026-08-29 on **479 audited findings** out of 1377, every one labelled by opening its cited `file:line`. Methodology, the full label set and per-finding verdicts are in `BENCHMARKING_RESULTS.md`, `PRECISION_AUDIT_V4.md` and `PRECISION_AUDIT_V3.md`.
...
**Go-only line-exact recall: 100.0%** — 401 of 401 in-scope `crypto/*` standard-library call sites, re-measured 2026-08-29 on the same 1244-finding dump (`dsa1_post.json`) the 94.7% precision figure above is sampled from, after a fix to the ground-truth builder itself (below). **This is a Go number and is not a recall figure for a seven-language tool**; no equivalent ground truth exists yet for the other six packs.
...
Every API in the ground truth, constructor and operation alike, is now at 100% in-scope recall. **The precision/recall trade this section used to describe no longer exists on this axis**: 94.7% precision and 100.0% recall are two different measurements of the same finding set, neither one bounding the other.
