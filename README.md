...
| Wall-clock time | 281s (4m 41s) for all 150 |
| Per project | median 200ms · mean 1872ms · p90 1.4s · max 137.6s |
...
`dump_findings.py` under the same binary, flags and corpus; the two agree at **1479** by
independent count, and that population is what the precision figure below is sampled from.

**Read the mean and the median as different facts.** The 9.4× gap between them is three
repositories: `aws-sdk-go-v2` alone takes 137.6s, and with `aws-sdk-go` and `wolfssl` the
top three account for over half the total wall-clock. **127 of 150 projects finish in under
...
`regression_check.py`, came in at **282.0s** against the 281.1s above; earlier passes over
the same corpus with the same flags gave 367.4s and 329.0s. Read the whole-corpus figure as
"between four and six minutes on two shared cores", not as three significant figures. The
finding counts do not move: both of today's passes produced exactly **1479**, and both of
the earlier ones produced exactly 1570 — the 91-finding difference is the `alg=none`
suppression recorded in `BENCHMARKING_RESULTS.md`, not run-to-run noise.
...
Audit-validated precision: **84.7%** (95% CI: 80.0%–89.4%) — measured 2026-08-28 on **227 audited findings** out of 1479, every one labelled by opening its cited `file:line`. Methodology, the full label set and per-finding verdicts are in `BENCHMARKING_RESULTS.md`, `PRECISION_AUDIT_V4.md` and `PRECISION_AUDIT_V3.md`.

**That is lower than the 86.5% this file published yesterday, and the scanner did not regress.** 86.5% carried 964 of 1570 findings — 61% of the corpus — at a constant 87.1%, taken from an audit whose per-row labels no longer identify any row. That stratum has now been re-sampled and re-labelled: 150 rows, uniform, seed 20260828, each read at its cited line. It audits at **76.6%**. Reading the labels instead of the constant moves *the same scan of the same corpus* from 86.5% to **80.0%**; this release's `alg=none` change then moves it to **84.7%**. Under the old constant the same release reads 87.3%. Both readings, the sample and the arithmetic are in `PRECISION_AUDIT_V4.md` § 3 — we publish the lower one because it is the one whose verdicts can be checked line by line.

**What that interval is.** A two-stratum weighted estimate over 227 findings audited by opening every cited `file:line` — 139 rows from the 887-finding stratum that has been scanned since the beginning, 88 from the 592-finding stratum restored in the 2026-08-27 corpus repair. The interval is the stratified normal approximation `Var = Σ wᵢ² pᵢ(1−pᵢ)/nᵢ`, not a Wilson interval on a pooled sample. The lower bound is 80.0%, so this is not a claim of 85%.

**What the denominator excludes.** `precision = TP / (TP + FP)`. The 227 audited rows are **188 TP, 34 FP and 5 DEPENDS**; the 5 DEPENDS rows — **2.2% of the sample** — are excluded from both sides rather than counted either way. A DEPENDS row is one whose operation is real but whose `algorithm_id` asserts a parameter the cited line does not state, typically an RSA modulus supplied by a caller. Scoring all 5 as false positives instead gives **82.9%**; scoring them all as true positives gives **85.1%**. That gap has narrowed from 5.2 points to 2.2 because the class DEPENDS names is being removed rather than tolerated — see below. Every figure quoted here uses the same exclusion, so they are comparable to each other — and any figure quoted against a scanner that uses a different convention is not.
...
**And why it then moved down again, on the same day.** The two gains above were measured with stratum A held at its constant, so neither of them could have been contradicted by that stratum even in principle. Auditing it contradicted both: 34 of its 150 sampled rows are false positives, and 24 of the 34 are two shapes — a JOSE algorithm constant compared or collected in Java (`alg.equals(JWSAlgorithm.PS256)`), and a call a test requires to fail (`jwt.encode(...)` inside `pytest.raises`). The third shape, an `alg=none` finding on a constant spelled "none", is the one this release removes: **91 findings, 5.8% of the corpus, every one carrying a critical severity hint and CWE-347 on a line that performs no authentication.** 92 fired, 1 was real. That is worth 4.7 points on the corrected estimate — 80.0% to 84.7% — and 0.8 on the old one, because 77 of the 91 sat inside the held constant where no measurement could see them.

### Recall, published beside precision

**Go-only line-exact recall: 74.4%** — 303 of 407 in-scope `crypto/*` standard-library call sites, re-measured 2026-08-28 on the 1479-finding dump the precision figure above is sampled from, and unchanged by the `alg=none` suppression: none of the 91 removed findings sits on a `crypto/*` call site. **This is a Go number and is not a recall figure for a seven-language tool**; no equivalent ground truth exists yet for the other six packs.
...
| Published precision (crypto findings) | 84.7% (227 audited rows, DEPENDS excluded) | ~49–76% (published benchmarks) | High (full data-flow) | Not published | Not published |
