...
Every API in the ground truth, constructor and operation alike, is now at 100% in-scope recall, re-measured directly against the current 2070-finding dump itself (`recall_check.py --dump` against a fresh `dump_findings.py` run at this cycle's binary, 441/441 in-scope both before and after). **The precision/recall trade this section used to describe no longer exists on this axis**: 97.33% precision and 100.0% recall are two measurements of the same 2070-finding dump, neither one bounding the other.
