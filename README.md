...
Audit-validated precision: **97.16%** (95% CI: 95.9%–98.5%) — measured 2026-08-31 on **635 audited findings** out of 1853, every one labelled by opening its cited `file:line`. Methodology, the full label set and per-finding verdicts are in `BENCHMARKING_RESULTS.md`, `PRECISION_AUDIT_V4.md` and `PRECISION_AUDIT_V3.md`.
...
- **Broader language coverage:** the Go rule pack is the least developed among the four largest packs today (93 classify arms vs. Java's 181, C/C++'s 124, and C#'s 117 — C/C++ grew past its earlier last-place position via the OpenSSL/liboqs PQC keygen and fetch coverage added across cycles 44–67). Expanding Go classify rules is the highest-leverage near-term coverage move.
