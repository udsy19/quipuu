...
**What the denominator excludes.** `precision = TP / (TP + FP)`. The 635 audited rows are **617 TP, 18 FP and 0 DEPENDS** — every DEPENDS-shaped ambiguity found in earlier samples has since been closed by the grounding fixes described below, so nothing is excluded on this pass. (This paragraph and the two above it describe the 693-row `#V5` sample as it stood 2026-09-01; `state/estimator.json`'s persisted audit constants were never advanced past their pre-`#V5` values, so the figure this file states today is measured against 635, not 693 — see `OPEN-ASK #ESTIMATORPERSIST` and this cycle's `#Y88` entry in `BENCHMARKING_RESULTS.md`.) A DEPENDS row, when one exists, is one whose operation is real but whose `algorithm_id` asserts a parameter the cited line does not state, typically an RSA modulus supplied by a caller. Every figure quoted here uses the same convention, so they are comparable to each other — and any figure quoted against a scanner that uses a different one is not.
...
- **Broader language coverage:** the Go rule pack is the least developed among the four largest packs today (105 classify arms vs. Java's 181, C/C++'s 126, and C#'s 118 — C/C++ grew past its earlier last-place position via the OpenSSL/liboqs PQC keygen and fetch coverage added across cycles 44–67). Expanding Go classify rules is the highest-leverage near-term coverage move.
