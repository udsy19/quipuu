...
Audit-validated precision: **97.22%** (95% CI: 96.0%–98.5%) — measured 2026-08-31 on **651 audited findings** out of 1853, every one labelled by opening its cited `file:line`. Methodology, the full label set and per-finding verdicts are in `BENCHMARKING_RESULTS.md`, `PRECISION_AUDIT_V4.md` and `PRECISION_AUDIT_V3.md`.
...
**What that interval is.** A two-stratum weighted estimate over 651 findings audited by opening every cited `file:line` — 271 rows from stratum A, 380 from stratum B (populations re-derived fresh this run at A=797, B=1056). The interval is the stratified normal approximation `Var = Σ wᵢ² pᵢ(1−pᵢ)/nᵢ`, not a Wilson interval on a pooled sample; its lower bound is 96.0%. Both strata's populations are re-derived fresh from the live dump on every run rather than carried forward, so they move cycle to cycle with `OPEN-ASK #CORPUSDRIFT`'s corpus-count drift independent of any detection change. This cycle's own 16 hand-verified true positives (OpenSSL's own `EVP_MD_fetch("SHA1"|"SHA256"|"SHA512"|"SHA3-256"|"SHA3-512", ...)` call sites, `#Y85`) land in stratum B, which is why its audited composition moved 355/364 → 371/380 while stratum A's (262/271) held.

**What the denominator excludes.** `precision = TP / (TP + FP)`. The 651 audited rows are **633 TP, 18 FP and 0 DEPENDS** — every DEPENDS-shaped ambiguity found in earlier samples has since been closed by the grounding fixes described below, so nothing is excluded on this pass. A DEPENDS row, when one exists, is one whose operation is real but whose `algorithm_id` asserts a parameter the cited line does not state, typically an RSA modulus supplied by a caller. Every figure quoted here uses the same convention, so they are comparable to each other — and any figure quoted against a scanner that uses a different one is not.
...
- **Broader language coverage:** the Go rule pack is the least developed among the four largest packs today (93 classify arms vs. Java's 181, C/C++'s 124, and C#'s 115 — C/C++ grew past its earlier last-place position via the OpenSSL/liboqs PQC keygen and fetch coverage added across cycles 44–67). Expanding Go classify rules is the highest-leverage near-term coverage move.
