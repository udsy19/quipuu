...
**What that interval is.** A two-stratum weighted estimate over 631 findings audited by opening every cited `file:line` — 271 rows from the 801-finding stratum A population, 360 from the 872-finding stratum B population. The interval is the stratified normal approximation `Var = Σ wᵢ² pᵢ(1−pᵢ)/nᵢ`, not a Wilson interval on a pooled sample; its lower bound is 95.8%. The stratum A population rose from 661 back to 801 between this measurement and the one before it — the same already-tracked, deferred `OPEN-ASK #CORPUSDRIFT`, present in the pre- and post-change dump equally and not an effect of this change, confirmed by scanning the unmodified pre-change binary against the same corpus state and getting the same 801/872 split. No finding moved this cycle, so the audited composition (262/271 stratum A, 351/360 stratum B) is unchanged from the measurement before it — only the population denominators did, which is why the figure moves without any TP/FP ratio having changed.
...
| Published precision (crypto findings) | 97.11% (631 audited rows, DEPENDS excluded) | ~49–76% (published benchmarks) | High (full data-flow) | Not published | Not published |
