...
**What that interval is.** A two-stratum weighted estimate over 613 findings audited by opening every cited `file:line` — 266 rows from the 656-finding stratum A population, 347 from the 853-finding stratum B population. The interval is the stratified normal approximation `Var = Σ wᵢ² pᵢ(1−pᵢ)/nᵢ`, not a Wilson interval on a pooled sample; its lower bound is 95.7%. The 5 openssl/openssl findings that move the headline to 97.09% are appended to this pool at their audited (100%) weight rather than re-stratified — see the caveat above — so the 618-row figure's 95.4–98.1% interval is not directly comparable to this one.

**What the denominator excludes.** `precision = TP / (TP + FP)`. The 618 audited rows are **600 TP, 18 FP and 0 DEPENDS** — every DEPENDS-shaped ambiguity found in earlier samples has since been closed by the grounding fixes described below, so nothing is excluded on this pass. A DEPENDS row, when one exists, is one whose operation is real but whose `algorithm_id` asserts a parameter the cited line does not state, typically an RSA modulus supplied by a caller. Every figure quoted here uses the same convention, so they are comparable to each other — and any figure quoted against a scanner that uses a different one is not.
...
| Published precision (crypto findings) | 97.09% (618 audited rows, DEPENDS excluded) | ~49–76% (published benchmarks) | High (full data-flow) | Not published | Not published |
