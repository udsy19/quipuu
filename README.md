...
Seven languages. Four output formats. No account. No cloud. No LLM. Median project scans in 190ms; the mean is 1724ms — see the benchmark table, both are real.
...
detection. Measured on the 150-project benchmark corpus (2070 findings,
2026-09-03): the two profiles produce the **same 2070 findings, identical
`algorithm_id` on every row**, of which **516 (24.9 %) land in a different
...
| Wall-clock time | 257.1s (4m 17s) for all 150 |
| Per project | median 190ms · mean 1724ms · p90 1.24s · max 141.0s |
| Languages covered | 7 (Go, Python, Java, JavaScript/TypeScript, C/C++, Rust, C#) |

Every row above except **Total findings** comes from **one timed run**: `python3 scan_corpus.py
--include-safe`, flags `--source --deps --include-safe`, profile `nist-default`, release build,
single-threaded, on **2 cores of an AMD EPYC 9354P with 7 GB RAM**, 2026-09-02, re-run after that
cycle's detection change (`is_call_asserted_to_fail` narrowed to the sole statement a
test-required-failure assertion actually wraps, `#Y119` — see `BENCHMARKING_RESULTS.md`). That
run's `results/all_findings.json` — the per-finding dump from `dump_findings.py` under the same
binary, flags and corpus — agreed with the timed run at **1907** by independent count that day,
once `dump_findings.py`'s own copy is filtered for the `crates-io:rustls-pemfile` symlink
duplicate the corpus's own `unscannable` declaration exists to prevent, which a prior cycle found
the script was silently ignoring (falling back to scanning the shared `rustls` clone under the
wrong project name when `scan_paths` resolved empty, rather than honoring
`scan_hints.unscannable`). **Total findings** above is **2070**, `#Y136`'s six new
legacy-OpenSSL-1.x rules plus `#Y137`'s one new WebCrypto `subtle.verify`/encapsulate-decapsulate
finding, both re-verified via a fresh `dump_findings.py` pass, 2026-09-03 — the
wall-clock/median/mean/p90/max figures have not been re-timed against the larger corpus and still
describe the 2026-09-02 run. That 2070-finding population is what the precision figure below is
sampled from.

**Read the mean and the median as different facts.** The gap between them is three
repositories: `aws-sdk-go-v2` alone takes 141.0s, and with `aws-sdk-go` (27.3s) and `wolfssl`
(15.1s) the top three are 71.4% of the total wall-clock. **131 of 150 projects finish in under
a second.** The mean describes a corpus deliberately stocked with vendored AWS SDKs; the
median describes a project. Neither is the number to quote alone. Wall-clock moved from the
prior run's 246.3s to 257.1s on the same 2-core box — read both as "a few minutes on two
shared cores," not as a real speed change; see the note below on run-to-run variance.

**Wall-clock on this box moves between runs.** Earlier passes over the same corpus with the
same flags gave 230.0s, 236.6s, 246.3s, 277.3s, 281.1s, 282.0s, 294.9s, 329.0s, 341.9s and 367.4s
against the 257.1s above — the 367.4s pass was over a wider corpus scope than this one. Read
the whole-corpus figure as "between four and six minutes on two shared cores", not as three
significant figures. The 2026-09-02 run's **1907** was reproduced exactly by an independent
`dump_findings.py` pass on the same binary, once that pass's own
`crates-io:rustls-pemfile` double-count (above) is filtered out — a pre-existing bug in the
script, not a corpus change. The steps down from
...
repair described there and below. Coverage commits since then took the total from 1056 to
1917, and a false-positive suppression — jsonwebtoken/PyJWT calls a test requires to fail,
`PRECISION_AUDIT_V4.md` § 6 — removed 11, down to 1906. **`#Y119`'s own fix recovered 1 of
those 11**: `is_call_asserted_to_fail` was suppressing every crypto call sharing a
`with pytest.raises(...):`/`expect(...).to.throw(...)` block with the call actually asserted
to fail, not only that call itself — collateral damage on any setup call sharing the block,
one real corpus instance of which (`npm/jsonwebtoken/test/wrong_alg.tests.js:46`, a genuine
successful `jwt.sign(...)` one line above the `jwt.verify(...)` the assertion requires to
throw) was restored, up to **1907**. `#Y136` (2026-09-03) then added 162 more via six new
`cpp.toml` legacy-OpenSSL-1.x rules, and `#Y137` (2026-09-03) added 1 more by giving
`subtle.verify`/`encapsulateKey`/`decapsulateKey`/`encapsulateBits`/`decapsulateBits` the same
WebCrypto coverage `generateKey`/`sign` already had — see the precision section below — up to
**2070**.
...
**What a fresh re-derivation looked like on 2026-09-01, historical — not what backs today's headline.** `#V5`'s own two-stratum weighted estimate over 693 findings audited by opening every cited `file:line` — 271 rows from stratum A, 422 from stratum B (populations re-derived fresh that run at A=797, B=1114). That interval was the stratified normal approximation `Var = Σ wᵢ² pᵢ(1−pᵢ)/nᵢ`, not a Wilson interval on a pooled sample; its lower bound was 96.2%. Both strata's populations are re-derived fresh from the live dump on every run rather than carried forward, so they move cycle to cycle with the corpus's own count drift between runs, independent of any detection change. **`#V5` itself (Go stdlib `crypto/mldsa` gaining coverage) added 58 corpus findings, all in stratum B, all hand-labelled true positive** — stratum A was untouched (271/271 rows, same as before). Stratum B's audited composition moved from 355/364 to **413/422**: the 58 added rows were the entire delta, and all 58 were TP, so its own precision rose slightly (97.5% → 97.9%) alongside the population growing that stratum's weight. `state/estimator.json`'s persisted audit constants were never advanced past their pre-`#V5` values, so this 693-row sample never became the figure this file states — see the next paragraph for what does.

**What the denominator excludes, and what actually backs today's headline.** `precision = TP / (TP + FP)`. This is the sample the **97.33%** headline above is measured against, and it is smaller and older-dated than the 693-row one just above, not a superset of it. The 798 audited rows are **777 TP, 21 FP and 0 DEPENDS** (`state/estimator.json`: `a_tp=266 a_fp=9`, `b_tp=511 b_fp=12`) — every DEPENDS-shaped ambiguity found in earlier samples has since been closed by the grounding fixes described below, so nothing is excluded on this pass. The 162-row jump from 635 to 797 is `#Y136`'s six new legacy-OpenSSL-1.x rules, hand-verified against corpus B (159 TP, 3 FP — full breakdown in `BENCHMARKING_RESULTS.md`), folded into the anchor sample because it spans six APIs across four independent codebases rather than one narrowly-scoped rule auditing its own near-tautological targets (the excluded shape from `#Y64`/`#Y65`, described above). The one-row jump from 797 to 798 is `#Y137`'s WebCrypto `subtle.verify`/encapsulate-decapsulate coverage, which surfaced exactly one new corpus finding (`npm:jose`'s `crypto.subtle.verify(algorithm, …)` in `src/lib/signing.ts:87`, a variable algorithm argument correctly classified `webcrypto-unattributed`), hand-verified TP, stratum A. `state/estimator.json`'s persisted audit constants were never advanced past their pre-`#V5` values, so the figure this file states today is measured against 798, not a superset of the 693-row `#V5` sample — still unresolved, see the `#Y88`/cycle-79 entries in `BENCHMARKING_RESULTS.md`. A DEPENDS row, when one exists, is one whose operation is real but whose `algorithm_id` asserts a parameter the cited line does not state, typically an RSA modulus supplied by a caller. Every figure quoted here uses the same convention, so they are comparable to each other — and any figure quoted against a scanner that uses a different one is not.
...
**Go-only line-exact recall: 100.0%** — 441 of 441 in-scope call sites (Go stdlib `crypto/*` plus `circl`'s third-party ML-DSA/ML-KEM packages, widened 2026-09-03, backlog #Y130), re-measured on the current 1907-finding corpus dump (`work/y128_default.json`), the dump the precision figure above is now sampled from. The ground truth previously carried zero coverage of Go 1.27's `crypto/mldsa` — the same stdlib package whose `MLDSA44`/`65`/`87` call sites this cycle's precision figure already credits with real findings — so the two headline numbers on this page were, until now, sampling different scanner surfaces. **This is a Go number derived from 25 real-world projects**; the cross-language number below is a smaller, planted probe, not the same kind of measurement.

Ground truth is built independently of our own rule files, by scanning the 25 Go corpus projects for 48 quantum-relevant APIs (36 Go stdlib, 12 `circl` — each cited to the package's own pkg.go.dev entry, never to `data/rules/go.toml`) and requiring the matching import, so it cannot inherit our blind spots. Reproduce with `python3 recall_check.py --clones DIR --dump results/all_findings.json`, which scores against a `dump_findings.py` artifact so recall is measured on exactly the finding set the precision audit samples.
...
Every API in the ground truth, constructor and operation alike, is now at 100% in-scope recall. **The precision/recall trade this section used to describe no longer exists on this axis**: on the 2070-finding dump above, 97.33% precision and 100.0% recall are two different measurements of the same finding set, neither one bounding the other. (The Go-only recall figures above were re-measured against `work/y128_default.json`, a 1907-finding dump predating this cycle's C/C++ coverage add — Go rules did not change this cycle, so the 441/441 in-scope count is unaffected, but the dump filename is stale; see `BENCHMARKING_RESULTS.md`'s `#Y136` entry.)

**A second denominator, which bounds the benchmark rather than the tool.** Those 441 sites are the ones inside the subtrees the harness actually hands to the scanner. Over the whole Go clone tree the ground truth is **1088 sites**, so **647 (59.5%) sit outside every scanned subtree and are never looked at**. The harness restricts 92 of 150 projects to `scan_hints.scan_paths`. Recall against the whole tree now reads **40.5%** (441/1088), and neither number should be quoted without saying which denominator it uses.
...
**Cross-language recall: 54.7% (64/117), measured 2026-09-03.** `benchmarks/corpus-a-ground-truth/`
...
`algorithm-table.toml` carries no MAC or password-KDF family yet, and `cpp` is 40.0% (6/15) real
coverage — `EVP_RSA_gen`/`EVP_PKEY_CTX_new_id` remain real, uncovered gaps, while the remaining 8
EVP-fetch sites are still probe artifacts no real C code would actually write. Not a CI gate —
narrowing an over-broad rule to kill a false positive can cost a true positive at the same site, so
recall is measured and published, never enforced as a floor.
...
| Published precision (crypto findings) | 97.33% (798 audited rows, DEPENDS excluded) | ~49–76% (published benchmarks) | High (full data-flow) | Not published | Not published |
| Published recall | 100.0% (Go stdlib + circl, 441/441 in-scope sites); 54.7% cross-language (64/117 planted sites, 7 languages) | Not published | Not published | Not published | Not published |
| Scan speed | 190ms median project; 257.1s for the 150-project corpus (2 cores) | Cloud-dependent | 5–15 min/repo | Not benchmarked | ~minutes |
...
- **Broader language coverage:** the Go rule pack is the least developed among the four largest packs today (109 classify arms vs. Java's 243, C/C++'s 137, and C#'s 131 — C/C++ grew past its earlier last-place position via the OpenSSL/liboqs PQC keygen and fetch coverage added across cycles 44–67). Expanding Go classify rules is the highest-leverage near-term coverage move.
