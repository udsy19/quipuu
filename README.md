...
Audit-validated precision: **97.17%** (95% CI: 95.9%–98.5%) — measured 2026-09-01 on **637 audited findings** out of 1853, every one labelled by opening its cited `file:line`. Methodology, the full label set and per-finding verdicts are in `BENCHMARKING_RESULTS.md`, `PRECISION_AUDIT_V4.md` and `PRECISION_AUDIT_V3.md`.
...
| Published precision (crypto findings) | 97.17% (637 audited rows, DEPENDS excluded) | ~49–76% (published benchmarks) | High (full data-flow) | Not published | Not published |
...
- **Broader language coverage:** the Go rule pack is the least developed among the four largest packs today (95 classify arms vs. Java's 181, C/C++'s 126, and C#'s 117 — C/C++ grew past its earlier last-place position via the OpenSSL/liboqs PQC keygen and fetch coverage added across cycles 44–67). Expanding Go classify rules is the highest-leverage near-term coverage move.
