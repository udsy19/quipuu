...
**What that interval is.** A two-stratum weighted estimate over 693 findings audited by opening every cited `file:line` — 271 rows from stratum A, 422 from stratum B (populations re-derived fresh this run at A=797, B=1114). The interval is the stratified normal approximation `Var = Σ wᵢ² pᵢ(1−pᵢ)/nᵢ`, not a Wilson interval on a pooled sample; its lower bound is 96.2%. Both strata's populations are re-derived fresh from the live dump on every run rather than carried forward, so they move cycle to cycle with the corpus's own count drift between runs, independent of any detection change. **This cycle's own change (Go stdlib `crypto/mldsa` gaining coverage, `#V5`) added 58 corpus findings, all in stratum B, all hand-labelled true positive** — stratum A is untouched (271/271 rows, same as before). Stratum B's audited composition moves from 355/364 to **413/422**: the 58 added rows are the entire delta, and all 58 are TP, so its own precision rises slightly (97.5% → 97.9%) alongside the population growing that stratum's weight.

**What the denominator excludes.** `precision = TP / (TP + FP)`. The 635 audited rows are **617 TP, 18 FP and 0 DEPENDS** — every DEPENDS-shaped ambiguity found in earlier samples has since been closed by the grounding fixes described below, so nothing is excluded on this pass. (This paragraph and the two above it describe the 693-row `#V5` sample as it stood 2026-09-01, plus subsequent cycles' own hand-labelled TPs; `state/estimator.json`'s persisted audit constants were never advanced past their pre-`#V5` values, so the figure this file states today is measured against 635, not 693 — still unresolved, see the `#Y88`/cycle-79 entries in `BENCHMARKING_RESULTS.md`.) A DEPENDS row, when one exists, is one whose operation is real but whose `algorithm_id` asserts a parameter the cited line does not state, typically an RSA modulus supplied by a caller. Every figure quoted here uses the same convention, so they are comparable to each other — and any figure quoted against a scanner that uses a different one is not.
