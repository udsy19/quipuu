...
Audit-validated precision: **97.15%** (95% CI: 95.8%–98.4%) — measured 2026-08-30 on **634 audited findings** out of 1687, every one labelled by opening its cited `file:line`. Methodology, the full label set and per-finding verdicts are in `BENCHMARKING_RESULTS.md`, `PRECISION_AUDIT_V4.md` and `PRECISION_AUDIT_V3.md`.
...
**What that interval is.** A two-stratum weighted estimate over 634 findings audited by opening every cited `file:line` — 271 rows from the 661-finding stratum A population, 363 from the 875-finding stratum B population. The interval is the stratified normal approximation `Var = Σ wᵢ² pᵢ(1−pᵢ)/nᵢ`, not a Wilson interval on a pooled sample; its lower bound is 95.9%. The stratum A population fell from 801 back to 661 between this measurement and the one before it — the same already-tracked, deferred `OPEN-ASK #CORPUSDRIFT`, present in the pre- and post-change dump equally and not an effect of this change, confirmed by scanning the unmodified pre-change binary against the same corpus state and getting the same 661/875 split. This cycle's own 3 hand-verified true positives (`SSL_CTX_set1_groups_list`) land in stratum B, which is why its audited composition moved 351/360 → 354/363 while stratum A's (262/271) held.

**What the denominator excludes.** `precision = TP / (TP + FP)`. The 634 audited rows are **616 TP, 18 FP and 0 DEPENDS** — every DEPENDS-shaped ambiguity found in earlier samples has since been closed by the grounding fixes described below, so nothing is excluded on this pass. A DEPENDS row, when one exists, is one whose operation is real but whose `algorithm_id` asserts a parameter the cited line does not state, typically an RSA modulus supplied by a caller. Every figure quoted here uses the same convention, so they are comparable to each other — and any figure quoted against a scanner that uses a different one is not.
...
| Published precision (crypto findings) | 97.15% (634 audited rows, DEPENDS excluded) | ~49–76% (published benchmarks) | High (full data-flow) | Not published | Not published |
