...
Audit-validated precision: **96.2%** (95% CI: 94.5%–97.9%) — measured 2026-08-29 on **473 audited findings** out of 1371, every one labelled by opening its cited `file:line`. Methodology, the full label set and per-finding verdicts are in `BENCHMARKING_RESULTS.md`, `PRECISION_AUDIT_V4.md` and `PRECISION_AUDIT_V3.md`.
...
**What that interval is.** A two-stratum weighted estimate over 194 findings audited by opening every cited `file:line` — 84 rows from the 469-finding stratum that has been scanned since the beginning, 110 from the 616-finding stratum restored in the 2026-08-27 corpus repair. The interval is the stratified normal approximation `Var = Σ wᵢ² pᵢ(1−pᵢ)/nᵢ`, not a Wilson interval on a pooled sample. The lower bound is 86.2%, so this is not a claim of 95%.

**What the denominator excludes.** `precision = TP / (TP + FP)`. The 194 audited rows are **171 TP, 18 FP and 5 DEPENDS**; the 5 DEPENDS rows — **2.6% of the sample** — are excluded from both sides rather than counted either way. A DEPENDS row is one whose operation is real but whose `algorithm_id` asserts a parameter the cited line does not state, typically an RSA modulus supplied by a caller. Scoring all 5 as false positives instead gives **88.1%**; scoring them all as true positives gives **90.7%**. The gap the DEPENDS convention is worth has narrowed from 2.8 points to 2.6 because the class it names is being removed rather than tolerated — see below. Every figure quoted here uses the same exclusion, so they are comparable to each other — and any figure quoted against a scanner that uses a different convention is not.
...
**Go-only line-exact recall: 98.5%** — 401 of 407 in-scope `crypto/*` standard-library call sites, re-measured 2026-08-29 on a 1244-finding dump (`dsa1_post.json`) that adds one rule pair for `crypto/dsa` on top of the operation-sites dump the 94.7% precision figure above is sampled from. `go.toml`'s sign/verify/encrypt/decrypt operation-site rules (see above) closed most of the gap this section used to describe; the `dsa.Sign`/`dsa.Verify` rule closes the rest of the operations row. **This is a Go number and is not a recall figure for a seven-language tool**; no equivalent ground truth exists yet for the other six packs.
...
|---|---|---|---|
| Generators and constructors (`rsa.GenerateKey`, `ecdsa.GenerateKey`, `ed25519.GenerateKey`, `dsa.GenerateKey`, `ecdh.*`, `md5.New`, `sha1.New`, `des.NewTripleDESCipher`, `rc4.NewCipher`) | 325 | 319 | **98.2%** |
...
**What is still missed, named rather than rounded away.** The three `ecdh.*` key-agreement APIs (`P256`, `P384`, `X25519`) each miss 2 of their sites — unrelated to the `dsa` fix, those rules predate it and were not touched. Every operation API in the table — every signer, verifier, encryptor, decryptor and one-shot digest — is now at 100%; the remaining 6-site gap is entirely in the constructor row, on `crypto/ecdh`. **The remaining precision/recall trade is now inverted from where this section started**: 94.7% precision and 98.5% recall are close to the same number from two directions, not the same architectural fact reported twice.

**A second denominator, which bounds the benchmark rather than the tool.** Those 407 sites are the ones inside the subtrees the harness actually hands to the scanner. Over the whole Go clone tree the ground truth is **1054 sites**, so **647 (61.4%) sit outside every scanned subtree and are never looked at**. The harness restricts 92 of 150 projects to `scan_hints.scan_paths`. Recall against the whole tree now reads **38.0%** (401/1054, up from 37.9%), and neither number should be quoted without saying which denominator it uses.
...
| Published precision (crypto findings) | 96.2% (473 audited rows, DEPENDS excluded) | ~49–76% (published benchmarks) | High (full data-flow) | Not published | Not published |
| Published recall | 98.5% (Go stdlib, 401/407 in-scope sites) | Not published | Not published | Not published | Not published |
