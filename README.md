...
detection. Measured on the 150-project benchmark corpus (2069 findings,
2026-09-03): the two profiles produce the **same 2069 findings, identical
`algorithm_id` on every row**, of which **516 (25.2 %) land in a different
...
`scan_hints.unscannable`). **Total findings** above is **2069**, `#Y136`'s six new
legacy-OpenSSL-1.x rules re-verified via a fresh `dump_findings.py` pass, 2026-09-03 — the
wall-clock/median/mean/p90/max figures have not been re-timed against the larger corpus and still
describe the 2026-09-02 run. That 2069-finding population is what the precision figure below is
sampled from.
...
`cpp.toml` legacy-OpenSSL-1.x rules — see the precision section below — up to **2069**.
...
Every API in the ground truth, constructor and operation alike, is now at 100% in-scope recall. **The precision/recall trade this section used to describe no longer exists on this axis**: on the 2069-finding dump above, 97.33% precision and 100.0% recall are two different measurements of the same finding set, neither one bounding the other. (The Go-only recall figures above were re-measured against `work/y128_default.json`, a 1907-finding dump predating this cycle's C/C++ coverage add — Go rules did not change this cycle, so the 441/441 in-scope count is unaffected, but the dump filename is stale; see `BENCHMARKING_RESULTS.md`'s `#Y136` entry.)
...
| Published precision (crypto findings) | 97.33% (797 audited rows, DEPENDS excluded) | ~49–76% (published benchmarks) | High (full data-flow) | Not published | Not published |
