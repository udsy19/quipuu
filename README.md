...
**Go-only line-exact recall: 100.0%** — 401 of 401 in-scope `crypto/*` standard-library call sites, measured 2026-08-29 on the 1244-finding dump (`dsa1_post.json`) that produced a since-superseded 94.7% precision figure, after a fix to the ground-truth builder itself (below). It has not been re-measured against the audit the precision figure above is now sampled from. **This is a Go number derived from 25 real-world projects**; the cross-language number below is a smaller, planted probe, not the same kind of measurement.
