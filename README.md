...
Seven languages. Four output formats. No account. No cloud. No LLM. Median project scans in 250ms; the mean is 2277ms — see the benchmark table, both are real.
...
| Wall-clock time | 341.9s (5m 42s) for all 150 |
| Per project | median 250ms · mean 2277ms · p90 1.67s · max 189.9s |
...
detection change (`#Y117`, oqs/liboqs-rust coverage). `results/all_findings.json` is the
per-finding dump from `dump_findings.py` under the same binary, flags and corpus; the two
agree at **1917** by independent count, ecosystem by ecosystem — once `dump_findings.py`'s
own copy is filtered for the `crates-io:rustls-pemfile` symlink duplicate the corpus's own
`unscannable` declaration exists to prevent, which this cycle found the script was silently
...
repositories: `aws-sdk-go-v2` alone takes 189.9s, and with `aws-sdk-go` (34.7s) and `wolfssl`
(19.4s) the top three are 71.4% of the total wall-clock. **123 of 150 projects finish in under
...
prior run's 236.6s to 341.9s on the same 2-core box — the finding counts did not, which is
the figure that matters here.

**Wall-clock on this box moves between runs.** Earlier passes over the same corpus with the
same flags gave 230.0s, 236.6s, 277.3s, 281.1s, 282.0s, 294.9s, 329.0s and 367.4s against the
341.9s above — the 367.4s pass was over a wider corpus scope than this one. Read the
whole-corpus figure as "between four and six minutes on two shared cores", not as three
significant figures. The finding counts do not move for timing reasons: this run's **1917**
is reproduced exactly by an independent `dump_findings.py` pass on the same binary, once that
pass's own `crates-io:rustls-pemfile` double-count (above) is filtered out — a pre-existing
bug in the script, not a corpus change. The steps down from
...
than removed findings, taking the total from 1056 to 1916, then, with a Java `Cipher.getInstance` false-positive
fix (a variable named `descriptor` matched the `(?i)DES` regex by coincidence of its own
identifier text, not its content — `#Y105`), down one to 1915, and now, with `oqs`
(liboqs-rust) `Kem::new`/`Sig::new` coverage (`#Y117`), up two to **1917**.
...
| Published precision (crypto findings) | 97.18% (637 audited rows, DEPENDS excluded) | ~49–76% (published benchmarks) | High (full data-flow) | Not published | Not published |
...
| Scan speed | 250ms median project; 341.9s for the 150-project corpus (2 cores) | Cloud-dependent | 5–15 min/repo | Not benchmarked | ~minutes |
