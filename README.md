...
**What that interval is.** A two-stratum weighted estimate over 630 findings audited by opening every cited `file:line` — 271 rows from the 801-finding stratum A population, 359 from the 871-finding stratum B population. The interval is the stratified normal approximation `Var = Σ wᵢ² pᵢ(1−pᵢ)/nᵢ`, not a Wilson interval on a pooled sample; its lower bound is 95.8%. The 7 openssl/openssl, aws/aws-lc and google/boringssl findings that move the headline to 97.14% are appended to stratum B's sample at their audited (100%) weight rather than re-stratified, the same treatment the 5 openssl/openssl keygen findings before them got — unlike the 5 liboqs findings, which were folded into a freshly re-derived stratum-A population and sample. So this interval is not directly comparable in sample composition to the one that preceded it, only in method.

**What the denominator excludes.** `precision = TP / (TP + FP)`. The 630 audited rows are **612 TP, 18 FP and 0 DEPENDS** — every DEPENDS-shaped ambiguity found in earlier samples has since been closed by the grounding fixes described below, so nothing is excluded on this pass. A DEPENDS row, when one exists, is one whose operation is real but whose `algorithm_id` asserts a parameter the cited line does not state, typically an RSA modulus supplied by a caller. Every figure quoted here uses the same convention, so they are comparable to each other — and any figure quoted against a scanner that uses a different one is not.
...
| Published precision (crypto findings) | 97.14% (630 audited rows, DEPENDS excluded) | ~49–76% (published benchmarks) | High (full data-flow) | Not published | Not published |
