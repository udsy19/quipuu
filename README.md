...
**What the denominator excludes, and what actually backs today's headline.** `precision = TP / (TP + FP)`. This is the sample the **97.18%** headline above is measured against, and it is smaller and older-dated than the 693-row one just above, not a superset of it. The 635 audited rows are **617 TP, 18 FP and 0 DEPENDS** (`state/estimator.json`: `a_tp=262 a_fp=9`, `b_tp=355 b_fp=9`) — every DEPENDS-shaped ambiguity found in earlier samples has since been closed by the grounding fixes described below, so nothing is excluded on this pass. `state/estimator.json`'s persisted audit constants were never advanced past their pre-`#V5` values, so the figure this file states today is measured against 635, not 693 — still unresolved, see the `#Y88`/cycle-79 entries in `BENCHMARKING_RESULTS.md`. A DEPENDS row, when one exists, is one whose operation is real but whose `algorithm_id` asserts a parameter the cited line does not state, typically an RSA modulus supplied by a caller. Every figure quoted here uses the same convention, so they are comparable to each other — and any figure quoted against a scanner that uses a different one is not.
...
| Published precision (crypto findings) | 97.18% (635 audited rows, DEPENDS excluded) | ~49–76% (published benchmarks) | High (full data-flow) | Not published | Not published |
...
- **Broader language coverage:** the Go rule pack is the least developed among the four largest packs today (106 classify arms vs. Java's 243, C/C++'s 131, and C#'s 131 — C/C++ grew past its earlier last-place position via the OpenSSL/liboqs PQC keygen and fetch coverage added across cycles 44–67). Expanding Go classify rules is the highest-leverage near-term coverage move.
