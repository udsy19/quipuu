...
**What that interval is.** A two-stratum weighted estimate over 631 findings audited by opening every cited `file:line` — 271 rows from the 661-finding stratum A population, 360 from the 872-finding stratum B population. The interval is the stratified normal approximation `Var = Σ wᵢ² pᵢ(1−pᵢ)/nᵢ`, not a Wilson interval on a pooled sample; its lower bound is 95.8%. The stratum A population fell from 801 to 661 between this measurement and the one before it — the already-tracked, deferred `OPEN-ASK #CORPUSDRIFT`, present in the pre- and post-change dump equally and not an effect of this change, confirmed by scanning the unmodified pre-change binary against the same corpus state and getting the same 661/872 split. The 1 `org.bouncycastle:bcprov-jdk18on` finding that moves the headline to 97.15% is appended to stratum B's sample at its audited (100%) weight rather than re-stratified, the same treatment the 7 OpenSSL keygen findings before it got. So this interval is not directly comparable in sample composition to the one that preceded it, only in method.

**What the denominator excludes.** `precision = TP / (TP + FP)`. The 631 audited rows are **613 TP, 18 FP and 0 DEPENDS** — every DEPENDS-shaped ambiguity found in earlier samples has since been closed by the grounding fixes described below, so nothing is excluded on this pass. A DEPENDS row, when one exists, is one whose operation is real but whose `algorithm_id` asserts a parameter the cited line does not state, typically an RSA modulus supplied by a caller. Every figure quoted here uses the same convention, so they are comparable to each other — and any figure quoted against a scanner that uses a different one is not.
...
| Published precision (crypto findings) | 97.15% (631 audited rows, DEPENDS excluded) | ~49–76% (published benchmarks) | High (full data-flow) | Not published | Not published |
