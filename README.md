...
**A second denominator, which bounds the benchmark rather than the tool.** Those 440 sites are the ones inside the subtrees the harness actually hands to the scanner. Over the whole Go clone tree the ground truth is **1087 sites**, so **647 (59.5%) sit outside every scanned subtree and are never looked at**. The harness restricts 92 of 150 projects to `scan_hints.scan_paths`. Recall against the whole tree now reads **40.5%** (440/1087), and neither number should be quoted without saying which denominator it uses.
...
| Published recall | 100.0% (Go stdlib, 440/440 in-scope sites); 49.6% cross-language (58/117 planted sites, 7 languages) | Not published | Not published | Not published | Not published |
