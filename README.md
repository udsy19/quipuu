...
**What that interval is.** A two-stratum weighted estimate over 623 findings audited by opening every cited `file:line` — 271 rows from the 801-finding stratum A population, 352 from the 864-finding stratum B population. The interval is the stratified normal approximation `Var = Σ wᵢ² pᵢ(1−pᵢ)/nᵢ`, not a Wilson interval on a pooled sample; its lower bound is 95.7%. The 5 liboqs findings that move the headline to 97.08% are folded into a freshly re-derived stratum-A population and sample, unlike the 5 openssl/openssl findings before them, which are appended to an earlier 613-row pool at their audited (100%) weight rather than re-stratified — so this interval is not directly comparable in sample composition to the one that preceded it, only in method.

**What the denominator excludes.** `precision = TP / (TP + FP)`. The 623 audited rows are **605 TP, 18 FP and 0 DEPENDS** — every DEPENDS-shaped ambiguity found in earlier samples has since been closed by the grounding fixes described below, so nothing is excluded on this pass. A DEPENDS row, when one exists, is one whose operation is real but whose `algorithm_id` asserts a parameter the cited line does not state, typically an RSA modulus supplied by a caller. Every figure quoted here uses the same convention, so they are comparable to each other — and any figure quoted against a scanner that uses a different one is not.
...
| Published precision (crypto findings) | 97.08% (623 audited rows, DEPENDS excluded) | ~49–76% (published benchmarks) | High (full data-flow) | Not published | Not published |
