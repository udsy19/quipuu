# quipuu V3 Corpus — Coverage Gaps Report
...
**Fix:** Extend `python_first_arg_call_method()` to also handle `identifier` nodes as the first positional argument, emitting the identifier name as a symbolic value. Update PY-010 (or add PY-011) in `quipuu/crates/core/data/rules/python.toml` to classify calls where the curve argument is a runtime variable.
...
3. Add corresponding classify rules to `quipuu/crates/core/data/rules/javascript.toml`.
