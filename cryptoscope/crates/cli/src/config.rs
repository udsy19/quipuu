//! `.cryptoscope.toml` schema and loader.
//!
//! Written by `cryptoscope init`; consumed by `cryptoscope scan` as
...
pub const CONFIG_FILENAME: &str = ".cryptoscope.toml";

/// Top-level config written by `cryptoscope init`.
...
fn default_html_out() -> String {
    "reports/cryptoscope.html".into()
}
fn default_sarif_out() -> String {
    "reports/cryptoscope.sarif".into()
}
fn default_cbom_out() -> String {
    "reports/cryptoscope.cbom.json".into()
}
fn default_summary_out() -> String {
    "reports/cryptoscope.summary.json".into()
}
...
/// Load `.cryptoscope.toml` from `dir`, if it exists.
///
...
mod tests {
    ...
    fn make_tempdir(suffix: &str) -> PathBuf {
        let base = std::env::temp_dir().join(format!("cryptoscope_config_test_{suffix}"));
