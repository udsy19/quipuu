//! cryptoscope-scan-certs — X.509 certificate scanner.
//!
...
//! `cryptoscope-core`, and emits [`Finding`]s per SPEC.md §6 (scan-certs).
//!
//! # Rule IDs
...
impl CertScanner {
    ...
    pub fn with_builtins() -> Result<Self, ScanError> {
        let builtins =
            cryptoscope_core::load_builtins().map_err(|e| ScanError::Parse(e.to_string()))?;
        Ok(Self {
    ...
    fn classify_cert(&self, cert: &X509Certificate<'_>, path: &Path) -> Vec<Finding> {
        ...
        let pk_usage = match pk_record.map(|r| r.primitive) {
            Some(Some(cryptoscope_core::Primitive::Kem))
            | Some(Some(cryptoscope_core::Primitive::KeyAgree)) => {
