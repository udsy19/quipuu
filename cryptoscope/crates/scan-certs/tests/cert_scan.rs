//! Integration tests for cryptoscope-scan-certs.
//!
//! Fixture certs live in `tests/fixtures/` and are pre-generated PEM/DER files.

use std::path::PathBuf;

use cryptoscope_scan_certs::{CertScanner, ScanError};
...
fn has_rule(findings: &[cryptoscope_core::Finding], rule_id: &str) -> bool {
    findings.iter().any(|f| f.rule_id == rule_id)
}

/// Helper: returns all findings with the given rule_id.
fn with_rule<'a>(
    findings: &'a [cryptoscope_core::Finding],
    rule_id: &str,
) -> Vec<&'a cryptoscope_core::Finding> {
