#!/usr/bin/env sh
# install.sh — cryptoscope installer
#
# Usage (once binary releases land):
#   curl -sSL https://raw.githubusercontent.com/<TBD>/cryptoscope/main/scripts/install.sh | sh
#
...
#   CRYPTOSCOPE_VERSION — pin a specific release tag  (default: latest)

set -eu

# ── Config ─────────────────────────────────────────────────────────────────
REPO_URL="https://github.com/<TBD>/cryptoscope"
RELEASES_URL="${REPO_URL}/releases"
INSTALL_DIR="${INSTALL_DIR:-/usr/local/bin}"
BIN_NAME="cryptoscope"
...
detect_os() {
  case "$(uname -s)" in
    Linux*)  echo "linux" ;;
    Darwin*) echo "macos" ;;
    *)       die "Unsupported OS: $(uname -s). Install manually: cargo install cryptoscope" ;;
  esac
}
...
detect_arch() {
  case "$(uname -m)" in
    ...
    *)                        die "Unsupported architecture: $(uname -m). Install manually: cargo install cryptoscope" ;;
  esac
}
...
target_triple() {
  ...
  case "${OS}-${ARCH}" in
    ...
    *)              die "No pre-built binary for ${OS}-${ARCH}. Install manually: cargo install cryptoscope" ;;
  esac
}
...
latest_version() {
  need curl
  VERSION=$(curl -sSf \
    "https://api.github.com/repos/<TBD>/cryptoscope/releases/latest" \
    | grep '"tag_name"' \
    | sed 's/.*"tag_name": *"\(.*\)".*/\1/')
...
main() {
  ...
  # │    cargo install --git https://github.com/<TBD>/cryptoscope          │
  # │                                                                       │
  # │  Or, if you have the repo cloned:                                     │
  # │    cargo install --path cryptoscope/crates/cli                       │
  # └───────────────────────────────────────────────────────────────────────┘
  printf '\n'
  printf '  cryptoscope pre-built binaries are not yet available.\n'
  printf '\n'
  printf '  Install from source with Cargo:\n'
  printf '    cargo install --git https://github.com/<TBD>/cryptoscope\n'
  printf '\n'
  printf '  Or clone and build locally:\n'
  printf '    git clone https://github.com/<TBD>/cryptoscope\n'
  printf '    cargo install --path cryptoscope/crates/cli\n'
  printf '\n'
  exit 0
  ...
  VERSION="${CRYPTOSCOPE_VERSION:-$(latest_version)}"
  # Strip leading 'v' for archive naming consistency.
  VER="${VERSION#v}"

  say "Installing cryptoscope ${VERSION} (${TARGET})"
  ...
  say "Done — cryptoscope ${VERSION} installed"
