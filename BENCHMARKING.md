# Proving cryptoscope: Evidence, Benchmarking, and Reproducibility
...
This is the only honest way to do this. Without the disagreement protocol, the team authoring the oracle has an obvious incentive to label every cryptoscope finding as ground truth and every competitor finding as noise. The disagreement log is published as part of the benchmark output.
...
A single page (rendered HTML, hosted at cryptoscope.dev/benchmarks) showing:
...
| W5 | `run_all.py` runner: cryptoscope + foxguard + CBOMkit + Semgrep |
...
10 weeks to "we have public, reproducible, peer-reviewed numbers proving cryptoscope is at least as good as incumbents on the axes that matter, and dramatically better on output validity, determinism, and deployment friction."
...
|---|---|
| cryptoscope will lead on **output schema validity** (we ship a validator; competitors mostly don't) | High |
| cryptoscope will lead on **throughput** (Rust + parallelism vs JVM/Python stacks) | High |
| cryptoscope will lead on **determinism** (P3 invariant: literal-traceable findings) | High |
| cryptoscope will lead on **deployment friction** (single static binary, no JVM/server) | Very high |
| cryptoscope will be **competitive but not dominant** on raw precision/recall vs CBOMkit on Java/Python | Medium |
| cryptoscope will **lose** to specialized tools on language-specific edge cases (e.g., jjwt enum resolution per V2) | High |
| cryptoscope will lead on **CycloneDX 1.7 support** (we ship it; CBOMkit is still 1.6 per the research) | High |
| cryptoscope will lead on **HNDL prioritization and risk scoring** (no incumbent does this) | Very high |
...
**Build the corpus, build the runner, publish the numbers, defend them in peer review — and the question "is cryptoscope as good as the incumbents" stops being a marketing claim and becomes a measurable, public, audit-grade fact.**
