...
[GitHub Security Advisories](https://github.com/udsy19/cryptoscope/security/advisories/new).
...
cryptoscope is a security tool that runs against untrusted source trees, so its own threat model
...
- **Code execution triggered by scanning a hostile input** — a crafted source file, certificate,
  or dependency manifest that causes cryptoscope to execute code, escape the target directory,
...
- **Denial of service from untrusted input** — a parser that hangs or exhausts memory on a crafted
  file. cryptoscope is expected to run in CI against arbitrary repositories.
...
- Missing hardening in a dependency with no demonstrated exploit path through cryptoscope.
