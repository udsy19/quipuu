#!/usr/bin/env sh
# install.sh — quipuu installer
#
# Usage (once binary releases land):
#   curl -sSL https://raw.githubusercontent.com/<TBD>/quipuu/main/scripts/install.sh | sh
#
...
#   QUIPUU_VERSION — pin a specific release tag  (default: latest)

set -eu

# ── Config ─────────────────────────────────────────────────────────────────
REPO_URL="https://github.com/<TBD>/quipuu"
...
detect_os() {
  case "$(uname -s)" in
    Linux*)  echo "linux" ;;
    Darwin*) echo "macos" ;;
    *)       die "Unsupported OS: $(uname -s). Install manually: cargo install quipuu" ;;
  esac
}
...
detect_arch() {
  case "$(uname -m)" in
    ...
    *)                        die "Unsupported architecture: $(uname -m). Install manually: cargo install quipuu" ;;
  esac
}
...
target_triple() {
  ...
  case "${OS}-${ARCH}" in
    ...
    *)              die "No pre-built binary for ${OS}-${ARCH}. Install manually: cargo install quipuu" ;;
  esac
}
...
latest_version() {
  need curl
  VERSION=$(curl -sSf \
    "https://api.github.com/repos/<TBD>/quipuu/releases/latest" \
    | grep '"tag_name"' \
    | sed 's/.*"tag_name": *"\(.*\)".*/\1/')
...
main() {
  ...
  # │    cargo install --git https://github.com/<TBD>/quipuu          │
  # │                                                                       │
  # │  Or, if you have the repo cloned:                                     │
  # │    cargo install --path quipuu/crates/cli                       │
  # └───────────────────────────────────────────────────────────────────────┘
  printf '\n'
  printf '  quipuu pre-built binaries are not yet available.\n'
  printf '\n'
  printf '  Install from source with Cargo:\n'
  printf '    cargo install --git https://github.com/<TBD>/quipuu\n'
  printf '\n'
  printf '  Or clone and build locally:\n'
  printf '    git clone https://github.com/<TBD>/quipuu\n'
  printf '    cargo install --path quipuu/crates/cli\n'
  printf '\n'
  exit 0
  ...
  VERSION="${QUIPUU_VERSION:-$(latest_version)}"
  # Strip leading 'v' for archive naming consistency.
  VER="${VERSION#v}"

  say "Installing quipuu ${VERSION} (${TARGET})"
  ...
  say "Done — quipuu ${VERSION} installed"
