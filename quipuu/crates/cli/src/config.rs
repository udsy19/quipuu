//! `.quipuu.toml` schema and loader.
//!
//! Written by `quipuu init`; consumed by `quipuu scan` as
...
pub const CONFIG_FILENAME: &str = ".quipuu.toml";

/// Top-level config written by `quipuu init`.
...
fn default_html_out() -> String {
    "reports/quipuu.html".into()
}
fn default_sarif_out() -> String {
    "reports/quipuu.sarif".into()
}
fn default_cbom_out() -> String {
    "reports/quipuu.cbom.json".into()
}
fn default_summary_out() -> String {
    "reports/quipuu.summary.json".into()
}
...
pub struct PolicyConfig {
    ...
    /// `Policy::load`; `quipuu policy list` names the built-in presets.
...
/// Load `.quipuu.toml` from `dir`, if it exists.
///
...
mod tests {
    ...
    fn make_tempdir(suffix: &str) -> PathBuf {
        let base = std::env::temp_dir().join(format!("quipuu_config_test_{suffix}"));
    ...
    fn partial_file_uses_field_defaults() {
        ...
        assert_eq!(config.output.html, "reports/quipuu.html");
