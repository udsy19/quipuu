...
use quipuu_core::{AlgorithmTable, Finding, Policy, ScanWarningKind, Severity, score_of};
...
pub fn emit_html(
    ...
    struct ScoredFinding<'a> {
        ...
        /// which is not a band. See `quipuu_core::score_of`.
        score: Option<u8>,
...
fn why_this_matters(
    algo: Option<&quipuu_core::AlgorithmRecord>,
    replacement: Option<&quipuu_core::AlgorithmRecord>,
) -> String {
    use quipuu_core::QuantumStatus;
    let Some(a) = algo else {
        return "Algorithm not in the quipuu catalogue; classification unavailable. \
