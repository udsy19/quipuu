//! quipuu-report — HTML, SARIF 2.1.0, and JSON summary emitters.
//!
...
use quipuu_core::{AlgorithmTable, Finding, Policy, ScanWarning};
...
/// SARIF. See [`quipuu_core::score_of`] for why `None` is not a band.
pub const UNSCORED_LABEL: &str = "Unscored";

/// Machine spelling of [`UNSCORED_LABEL`] — the HTML report's CSS class.
///
/// Deliberately not a [`quipuu_core::Severity`] slug: `--fail-on` parses those,
