...
fn export_cbom(app: &mut Tui) {
    use quipuu_cbom::emit::ScanTarget;
    use quipuu_cbom::{EmitOptions, emit_cbom_json};

    let opts = EmitOptions::new(
        ScanTarget {
            name: "quipuu-scan".to_string(),
            version: None,
        },
...
fn draw(frame: &mut ratatui::Frame, app: &Tui, kpi: &Kpi) {
    ...
    frame.render_widget(render_header("quipuu", env!("CARGO_PKG_VERSION")), outer[0]);
