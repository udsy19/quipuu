//! SLH-DSA KAT verifier (FIPS 205 / NIST ACVP).
//!
//! # P4 invariant
//! This module DOES NOT run any SLH-DSA implementation. It receives
...
//!
//! # Supported parameter sets (v0.1)
//! - SLH-DSA-SHAKE-128s
//!
//! Additional parameter sets (SLH-DSA-SHA2-128f, SLH-DSA-SHAKE-256s, etc.)
//! can be added by dropping JSON files into `data/acvp-vectors/` and
//! extending `vectors.rs` and the match arm below.
...
/// Dispatch SLH-DSA verification by parameter set and mode.
pub fn run(
    parameter_set: &str,
    mode: &str,
    ...
    match (parameter_set, mode) {
        ("SLH-DSA-SHAKE-128s", "keyGen") => {
            let vd = vectors::slh_dsa_shake_128s_keygen();
            verify_fields(
                "SLH-DSA",
                parameter_set,
                mode,
        ...
        ("SLH-DSA-SHAKE-128s", "sigGen") => {
            let vd = vectors::slh_dsa_shake_128s_siggen();
            verify_fields(
                "SLH-DSA",
                parameter_set,
                mode,
        ...
        (ps, m) => Err((
            E_RULESET_INVALID,
            format!(
                "SLH-DSA: unsupported parameter_set/mode \"{ps}/{m}\". \
                 Supported (v0.1): SLH-DSA-SHAKE-128s/keyGen, SLH-DSA-SHAKE-128s/sigGen"
            ),
        )),
    }
}
