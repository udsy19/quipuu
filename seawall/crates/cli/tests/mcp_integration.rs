//! Integration tests for the `seawall mcp-serve` subcommand.
//!
...
fn binary_path() -> PathBuf {
    // The integration test binary lives alongside seawall in target/debug/.
...
impl McpSession {
    fn start(allow_network: bool) -> Self {
        ...
        let mut child = cmd.spawn().expect("failed to spawn seawall mcp-serve");
...
fn test_initialize_handshake() {
    ...
    assert_eq!(resp["result"]["serverInfo"]["name"], "seawall-mcp");
