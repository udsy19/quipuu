...
fn high_finding_tree(suffix: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("seawall_fail_on_{suffix}"));
...
fn clean_tree(suffix: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("seawall_fail_on_clean_{suffix}"));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).expect("create fixture dir");
...
fn scan(args: &[&str]) -> Output {
    Command::new(binary_path())
        ...
        .args(args)
        .output()
        .expect("failed to run seawall scan")
}
...
/// Without `--fail-on`, the exit code says only whether seawall ran. That is
...
fn missing_path_is_fatal() {
    let missing = std::env::temp_dir().join("seawall_fail_on_definitely_absent");
