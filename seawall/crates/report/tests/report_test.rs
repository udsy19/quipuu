//! Integration tests for the `seawall-report` crate.
//!
...
use seawall_core::{ScanWarning, ScanWarningKind, load_builtins};
use seawall_report::{ReportOptions, emit_html, emit_sarif, emit_summary_json};
...
fn sarif_critical_finding_maps_to_error_and_9_0() {
    use seawall_core::{Exposure, Severity, UsageContext};
    ...
    let critical_rule_id = {
        ...
        let score = seawall_core::QuantumRiskScore::compute(rsa2048, algo, &policy);
        assert_eq!(
            score.severity,
            Severity::Critical,
...
fn finding_for(algo_id: &str) -> seawall_core::Finding {
    use seawall_core::{Confidence, Exposure, Finding, Location, UsageContext};
    Finding {
...
fn phase2_partition_audible_separates_inventory_from_real_findings() {
    use seawall_report::partition_audible;

    let b = seawall_core::load_builtins().expect("builtins load");
    let findings = vec![
...
fn phase2_sarif_excludes_suppressed_when_audible_subset_passed() {
    ...
    let displayed: Vec<seawall_core::Finding> = audible.iter().map(|f| (*f).clone()).collect();
