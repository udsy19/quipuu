...
pub struct RawMatch {
    ...
    pub site_context: seawall_core::SiteContext,
}
...
impl Scanner {
    ...
    fn scan_file_into(&self, path: &Path, out: &mut Vec<Finding>) -> Result<(), ScanError> {
        ...
        // Size cap before reading. seawall runs in CI against arbitrary
...
/// the whole scan rather than skipping one file. seawall runs in CI against
...
fn match_call(call: Node<'_>, source: &[u8], language: Language) -> Option<RawMatch> {
    ...
    if kind == "invocation_expression" {
        ...
        return Some(RawMatch {
            ...
            site_context: seawall_core::SiteContext::Call,
        });
    }
...
fn match_go_tls_min_version(keyed: Node<'_>, source: &[u8]) -> Option<RawMatch> {
    ...
    Some(RawMatch {
        ...
        site_context: seawall_core::SiteContext::StructLiteral,
    })
}
...
pub(crate) fn classify_site_context(
    ...
    source: &[u8],
    language: Language,
) -> seawall_core::SiteContext {
    use seawall_core::SiteContext;
