...
use seawall_core::{QuantumRiskScore, Severity, load_builtins};
...
fn end_to_end_rsa_keygen_scores_high() {
    ...
    rsa2048.usage_context = seawall_core::UsageContext::KeyEstablishmentLongLived;
    rsa2048.exposure = seawall_core::Exposure::PublicInternet;
...
fn phase6_unreadable_file_becomes_warning_not_error() {
    use seawall_core::ScanWarningKind;
    use std::fs;
    use std::os::unix::fs::PermissionsExt;

    // Build a fresh per-test temp dir (no external tempfile crate dep).
    let tmp =
        std::env::temp_dir().join(format!("seawall-phase6-{}-{}", std::process::id(), line!()));
...
use seawall_scan_source::{ClassifyRule, RulePack};
...
fn phase13_hash_variant_consistency() {
    ...
    for (lang, r) in &rules {
        ...
        for am in r.when.args.values() {
            if let seawall_scan_source::rules::ArgMatch::Regex(rx) = am {
                probe.push(' ');
                probe.push_str(&rx.regex);
            } else if let seawall_scan_source::rules::ArgMatch::ExactStr(s) = am {
                probe.push(' ');
                probe.push_str(s);
            }
        }
...
fn phase14b_every_when_args_regex_compiles() {
    use regex::Regex;
    use seawall_scan_source::rules::ArgMatch;
    let rules = all_classify_rules();
    let mut bad = Vec::new();
    for (lang, r) in &rules {
...
fn phase14b_classify_rule_ids_unique_within_language() {
    ...
    use seawall_scan_source::RulePack;
