...
## Knowledge Reference for cryptoscope/scan-certs
...
9. [Comprehensive OID Table for cryptoscope Risk Engine](#9-comprehensive-oid-table-for-cryptoscope-risk-engine)
10. [DECISIONS for cryptoscope/scan-certs](#10-decisions-for-cryptoscopescan-certs)
...
**Decision for cryptoscope:** Do NOT hardcode Falcon/FN-DSA OIDs. Add a comment in the OID table noting that values in `2.16.840.1.101.3.4.3.{32..34}` (the gap after SLH-DSA) are likely candidates. Watch NIST CSOR for finalization.
...
**Why:** cryptoscope's live-host scanning mode (`--host`) will encounter these in the wild. Understanding what is deployed tells us which OIDs need to be recognized first.
...
Operated by the Open Quantum Safe project; supports all PQC algorithms from liboqs including ML-KEM, ML-DSA, SLH-DSA, Falcon hybrid combinations. This is the primary interop testing target for cryptoscope's live-host mode.
...
**cryptoscope implication:** When a composite OID (1.3.6.1.5.5.7.6.37–66) is seen in `signatureAlgorithm`, report it as "HYBRID-COMPOSITE: PQC + Classical". Both component algorithms should be decoded and their classical component risk-classified independently.
...
**cryptoscope implication:** If the scanner sees OID `2.16.840.1.114027.80.6.1` in extensions, report "CHAMELEON-CERT: delta certificate descriptor present; base key is classical, delta key likely PQC (or vice versa)".
...
**cryptoscope implication:** Look for non-standard extensions with PQC OIDs in the extension value. No standardized OID arc as of mid-2026.
...
**MSRV:** Rust 1.67.1  
**Recommendation for cryptoscope:** PRIMARY parsing crate.
...
**Feature flags relevant to cryptoscope:**
...
**Recommendation for cryptoscope:** Use as SUPPORTING crates for re-encoding and type-level OID handling, not as the primary parser.
...
**cryptoscope use:** Not directly useful for scanning — it is a runtime verification crate, not a parsing crate.
...
**cryptoscope use:** Useful for classical chain validation to confirm a chain is structurally valid. Will return `UnsupportedSignatureAlgorithm` for any PQC cert — which is itself informative signal for the scanner.
...
**cryptoscope use:** Lower priority. x509-parser is more actively maintained and more widely used.
...
**cryptoscope use:** An alternative to x509-parser if `#[no_std]` support is needed (e.g., WASM target). Not the primary recommendation.
...
Custom PQC OIDs can be inserted at runtime with `registry.insert(oid, OidEntry::new("ML-DSA-65", "FIPS 204"))`. This is the correct approach for cryptoscope — build the PQC OID table in Rust code and register at startup.
...
A full X.509 chain consists of: end-entity certificate, zero or more intermediate CA certificates, and a trust anchor (root CA). Each level has its own `signatureAlgorithm` and `subjectPublicKeyInfo`. For a complete risk picture, cryptoscope must report all three independently.
...
## 9. Comprehensive OID Table for cryptoscope Risk Engine

This is the authoritative lookup table cryptoscope should hardcode. Classification levels:
...
## 10. DECISIONS for cryptoscope/scan-certs
