...
Every load-bearing build decision for cryptoscope, in **Why → Evidence → Decision** form. Each entry links back to the knowledge folder that established the evidence. If new evidence later contradicts a decision, update it here first, then ripple through SPEC.md.
...
**Decision:** Every cryptoscope finding emits:
...
- Cross-ref CBOM via `properties.cryptoscope/cbom-ref` on each result (set the convention).
- No `fix` objects in v1 (PQC migration is not a mechanical substitution).
- Document GitLab 18.11+ requirement in the README; ship a converter sub-command (`cryptoscope report --format gitlab-sast`) for older GitLab.
...
## D-13 — Working name: **KEEP `cryptoscope`** (resolved 2026-06-12, see `verify-resolution.md` V-05)
...
**Evidence (resolved):**
- crates.io API for `cryptoscope` → **available**, no crate published.
- GitHub: 119 repos search-match `cryptoscope`, but the top by stars is a 1-star cryptocurrency trading bot. None are crypto-asset/CBOM scanners.
...
**Decision:** **Keep `cryptoscope`.** Register the crates.io name and the GitHub org **before** any public code lands. Pinned backup name: **`cbomx`** (short, descriptive, distinctive) if `cryptoscope` becomes contested at publication time.
...
| D-13 | Rename — "cryptoscope" collides with existing tools | 08-competitors |
