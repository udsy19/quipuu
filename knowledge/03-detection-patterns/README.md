# Detection Patterns for cryptoscope

> **Purpose**: Authoritative reference for rule design in the cryptoscope Rust/tree-sitter crypto scanner. Covers existing rule formats we can borrow, high-value API targets per language, parameter extraction patterns, and the competitive landscape.
...
7. [Proposed cryptoscope Rule Schema](#7-proposed-cryptoscope-rule-schema)
...
**Takeaway**: The Java DSL is expressive but not portable. Key ideas worth capturing in cryptoscope's YAML schema: context types (cipher/key/sign/tls/kem), recursive parameter dependencies, factory-based value extraction, library attribution.
...
**Takeaway**: Adopt the `(field, operator, value)` pattern language for cryptoscope's classification layer. Extend the field set beyond the four cryptobom-forge fields to add `language`, `library`, `confidence`, `curve`, `iv_source`.
...
**Takeaway**: Confirms the fragmentation problem cryptoscope addresses. No new schema to adopt.
...
### 5.3 What Semgrep and CodeQL Miss (cryptoscope's Opportunity Space)
...
### 6.4 Proposed cryptoscope Confidence Encoding
...
Store in the SARIF `result.properties` map as `"confidence": "high"` alongside `"cryptoscope/keylen": "2048"` and `"cryptoscope/algo": "RSA"`.
...
## 7. Proposed cryptoscope Rule Schema
...
| CycloneDX CBOM 1.6 | `bom-1.6.schema.json` (local) | Output schema for cryptoscope CBOM |
